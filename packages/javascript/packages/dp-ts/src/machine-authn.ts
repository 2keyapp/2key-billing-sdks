/**
 * HTTP client for `/api/v1/machine-authn/*` on the billing host.
 * Device crypto (CSR, sign, mTLS, PoP) stays in `@2key/dp-mtls`.
 */
...
/**
 * Creates a billing-host Machine AuthN HTTP client.
 */
export function createMachineAuthnClient(options: MachineAuthnClientOptions) {
  ...
  return {
    register(body: Record<string, unknown>) {
      return request<Record<string, unknown>>("POST", `${MACHINE_AUTHN}/register`, body);
    },
    ...
    assertSubset(body: Record<string, unknown>) {
      return request<{ ok: boolean }>("POST", `${MACHINE_AUTHN}/assert-subset`, body);
    },
    platformRoot() {
      return request<Record<string, unknown>>(
        "GET",
