...
**Canonical pure algebra:** `@2key/dp-authorize` (TS, this repo) + `dp_rust::authorize` (Rust, `2key-core-sdk`)  
**Conformance:** `conformance/dp-authz/fixtures.json` (canonical here; Rust CI must stay in sync)
...
| Package | Path | Role |
|---------|------|------|
| `@2key/dp-authorize` | `packages/javascript/packages/dp-authorize` | Pure authorize + subset + `enforceLocally` |
| `dp-rust` (`authorize` mod) | `2key-core-sdk` | Same algebra for CLI/agents |
| `@2key/dp-ts` | `packages/javascript/packages/dp-ts` | TS clients (call algebra before HTTP) |
...
| Catalogs | `packages/javascript/catalogs/*` | Tenant seeds |
