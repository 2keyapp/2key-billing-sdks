...
**`packages/javascript` Delegate Permissions packages do not persist secrets.** They accept in-memory `DeviceIdentity` / JWKs / PEMs only. That includes `dp-ts`, `dp-mtls`, and `@2key/browser-sdk`. Rust counterparts (`dp-rust*`) live in `2key-core-sdk` with the same policy.

The **CLI and agent are the host** for a laptop or server (in `2key-core-sdk`): they write secrets under `$DP_STATE_DIR` (default `~/.{product}`):
